...
export function splitSyllables(word: string): string[] {
  ...
  for (let i = 0; i < word.length; i++) {
    ...
    if (isVowel(char)) {
      if (nextChar && !isVowel(nextChar) && nextNextChar && isVowel(nextNextChar)) {
        syllables.push(currentSyllable);
        currentSyllable = '';
      } else if (nextChar && !isVowel(nextChar) && nextNextChar && !isVowel(nextNextChar)) {
        // Two consonants. Check for inseparable groups like 'bl', 'tr', etc.
        const pair = nextChar + nextNextChar;
        const inseparable = ['bl', 'cl', 'fl', 'gl', 'pl', 'br', 'cr', 'dr', 'fr', 'gr', 'pr', 'tr', 'ch', 'll', 'rr'];
        ...
        if (nextChar === 'n' && nextNextChar === 's' && nextNextNextChar && !isVowel(nextNextNextChar)) {
...
export function getWordType(syllables: string[], tonicIndex: number): 'aguda' | 'llana' | 'esdrujula' {
